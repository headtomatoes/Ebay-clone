      ...
      function Accordion({title, children}) {
        ...
        return (
            <div className="border rounded-lg overflow-hidden">
              ...
              <div
                  className={`transition-all duration-300 ease-in-out px-4 text-sm text-gray-700 ${open ? 'max-h-[500px] py-3 opacity-100' : 'max-h-0 py-0 opacity-0'} overflow-hidden`}>
                {children}
              </div>
            </div>
        );
              ...
              <div className="w-full md:w-1/2 flex flex-col justify-between min-h-[500px]">
                ...
                {user?.roles?.includes('ROLE_SELLER') && user?.username === product.sellerUsername && (
                    <div className="flex space-x-4 mt-6">
                      ...
                      <button onClick={() => navigate(`/seller/products/update/${product.productId}`)}
                              className="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">Update
                      </button>
                      <button onClick={() => navigate(`/seller/auction/create/${product.productId}`)}
                              className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Create Auction
                      </button>
                    </div>
                )}
              </div>
              ...
              {user ? (
                ...
                />
                    ...
                    <button onClick={handleReviewSubmit}
                            className="py-2 px-6 bg-blue-600 text-white rounded-md">Submit Review
                    </button>
                  </div>
              ) : (
